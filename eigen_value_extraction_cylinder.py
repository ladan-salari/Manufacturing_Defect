...
# Input: ABAQUS simulation results (.odb file) from buckling simulation
...
while sec_num <= 4:
    while bar_num <= bar_indexes[sec_num - 1]:
        odb_directory = (
            "C:/Users/ladan/Documents/Research/Nano_CT_Bars_FEA/Cylinder_bars/Sec%d-bar%d-mid-PD-Final.odb"
            % (sec_num, bar_num)
        )
        ...
        try:
            ...
            text_information = "\n Sec%d-bar%d-mid   First Eigen Value=  %f" % (
                sec_num,
                bar_num,
                File_eigenvalue,
            )
            file = open("Ladans_Eigen_Values.txt", "a")
